  ...
  return (
    <div className="min-h-screen bg-background p-6 md:p-12 text-foreground transition-colors duration-300">
      <header className="mb-16 flex flex-col lg:flex-row lg:items-end justify-between gap-8 mt-16 md:mt-0">
        ...
        <form onSubmit={addTicker} className="flex items-center gap-3 bg-card/50 p-2 rounded-2xl border border-foreground/5 backdrop-blur-xl shadow-xl w-full lg:w-auto">
          <div className="relative flex-1 lg:flex-none">
            ...
            <input 
              ...
              className="bg-background/50 border border-foreground/10 rounded-xl pl-11 pr-4 py-2.5 text-xs font-bold w-full lg:w-64 outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all font-mono placeholder:opacity-30"
            />
          </div>
          <button 
            type="submit" 
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="h-24 bg-card/20 border border-foreground/5 rounded-[2rem] animate-pulse" />
          ))}
        </div>
      ) : error ? (
        ...
        <div className="flex flex-col items-center justify-center min-h-[40vh] border border-dashed border-foreground/10 rounded-[3rem] bg-card/10 px-6 text-center text-foreground">
        ...
        <div className="space-y-4">
          {values?.docs.map(doc => {
            const data = doc.data();
            return (
              <div key={doc.id} className="group flex items-center justify-between bg-card/30 backdrop-blur-sm border border-foreground/5 px-8 py-6 rounded-[2.5rem] hover:border-foreground/20 transition-all duration-500 relative overflow-hidden">
                ...
                <div className="flex items-center gap-8 relative z-10">
                  <div className="w-12 h-12 bg-foreground/5 border border-foreground/10 rounded-2xl flex items-center justify-center">
                ...
                <div className="flex items-center gap-12 relative z-10">
                  ...
                  <button 
                    ...
                    className="p-3 bg-foreground/5 border border-foreground/10 rounded-2xl text-foreground/20 hover:text-rose-500 hover:border-rose-500/20 hover:bg-rose-500/5 transition-all active:scale-95"
                  >
